#!/bin/bash
	...
	zenity --question --title "Sublime Text 3 installation" --text "This script will install Sublime Text 3 trial on your computer"
	if [ $? = 1 ]; then
		exit 0
	fi
	...
	URL="`grep -Po '(?<=href="http://c758482.r82.cf2.rackcdn.com/sublime_text_3_build_)(.*?)([^x64].tar.bz2)(?=">)' <<<"$STURLTOCHECK"`"
	...
	#rename binary
	if [ -f "$THEFILE" ]; then
		mv "$THEFILE" /home/$USER/sublime_text.tar.bz2
	fi
    sleep 1s
	...
	#Install icon
        gksudo cp "/home/$USER/sublime_text_3/Icon/128x128/sublime-text.png" /usr/share/icons/hicolor/128x128/apps/
       	gksudo update-icon-caches /usr/share/icons/hicolor


#    /home/$USER/.infinity/startmenu-default EDITOR "Sublime Text 3" "\"/home/$USER/sublime_text_3/sublime_text\""
	/opt/infinity/startmenu/startmenu-add "Sublime Text 3" "/home/$USER/sublime_text_3/sublime_text" "sublime-text"
    
    zenity --info --title="Sublime Text 3 installation" --text "The installation was successfull"
